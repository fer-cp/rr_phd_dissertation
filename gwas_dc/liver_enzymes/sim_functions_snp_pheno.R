...
eigvec <- function(X, b, covariates = NULL) {
  ...
  if (is.null(covariates)) {
    ...
    Vres <- V
    for (j in 1:q) {
      Ures[, j] <- lm(U[, j] ~ covariates)$residuals
      Vres[, j] <- lm(V[, j] ~ covariates)$residuals
    }

    a11 <- n / (n + 1) * sapply(1:q, function(u) var(Ures[, u]))
    a12 <- n / (n + 1) * sapply(1:q, function(u) cov(Ures[, u], Vres[, u]))
    a22 <- n / (n + 1) * sapply(1:q, function(u) var(Vres[, u]))
    a12sq <- a12^2
